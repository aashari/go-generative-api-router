...
# 
# Detects:
# - AWS Account IDs, Access Keys, Secret Keys, Resource IDs
# - OpenAI API Keys (sk-...), Google/Gemini API Keys (AIza...)
# - Telegram Bot Tokens (format: 123456789:ABC-DEF1234ghIkl-zyx57W2v1u123ew11)
# - LINE Bot Tokens and Channel Access Tokens
...
echo "6. Checking for Telegram Bot Tokens..."
TELEGRAM_TOKEN_MATCHES=$(grep -r -E '[0-9]{8,10}:[a-zA-Z0-9_-]{35}' \
    --exclude-dir=.git \
    --exclude-dir=node_modules \
    --exclude-dir=vendor \
    --exclude-dir=.terraform \
...
if [ -n "$TELEGRAM_TOKEN_MATCHES" ]; then
    # Check if matches are only in gitignored files
    GITIGNORED_TELEGRAM_MATCHES=$(echo "$TELEGRAM_TOKEN_MATCHES" | grep -E '\./configs/credentials\.json|\./\.env' || true)
    NON_GITIGNORED_TELEGRAM_MATCHES=$(echo "$TELEGRAM_TOKEN_MATCHES" | grep -v -E '\./configs/credentials\.json|\./\.env' || true)
    
    if [ -n "$NON_GITIGNORED_TELEGRAM_MATCHES" ]; then
        report_issue "Telegram Bot Tokens found in tracked files:"
        echo "$NON_GITIGNORED_TELEGRAM_MATCHES"
        echo ""
    fi
    
    if [ -n "$GITIGNORED_TELEGRAM_MATCHES" ]; then
        report_warning "Telegram Bot Tokens found in gitignored files (this is expected):"
        echo "$GITIGNORED_TELEGRAM_MATCHES"
        echo ""
    fi
    
    if [ -z "$NON_GITIGNORED_TELEGRAM_MATCHES" ]; then
        report_success "Telegram Bot Tokens only found in gitignored files (safe)"
    fi
else
    report_success "No Telegram Bot Tokens found in tracked files"
fi

echo "7. Checking for LINE Bot Tokens..."
LINE_TOKEN_MATCHES=$(grep -r -E '(line_channel_access_token|LINE_CHANNEL_ACCESS_TOKEN|[a-zA-Z0-9+/]{86}==)' \
    --exclude-dir=.git \
    --exclude-dir=node_modules \
    --exclude-dir=vendor \
    --exclude-dir=.terraform \
...
if [ -n "$LINE_TOKEN_MATCHES" ]; then
    # Check if matches are only in gitignored files
    GITIGNORED_LINE_MATCHES=$(echo "$LINE_TOKEN_MATCHES" | grep -E '\./configs/credentials\.json|\./\.env' || true)
    NON_GITIGNORED_LINE_MATCHES=$(echo "$LINE_TOKEN_MATCHES" | grep -v -E '\./configs/credentials\.json|\./\.env' || true)
    
    if [ -n "$NON_GITIGNORED_LINE_MATCHES" ]; then
        report_issue "LINE Bot Tokens found in tracked files:"
        echo "$NON_GITIGNORED_LINE_MATCHES"
        echo ""
    fi
    
    if [ -n "$GITIGNORED_LINE_MATCHES" ]; then
        report_warning "LINE Bot Tokens found in gitignored files (this is expected):"
        echo "$GITIGNORED_LINE_MATCHES"
        echo ""
    fi
    
    if [ -z "$NON_GITIGNORED_LINE_MATCHES" ]; then
        report_success "LINE Bot Tokens only found in gitignored files (safe)"
    fi
else
    report_success "No LINE Bot Tokens found in tracked files"
fi

echo "8. Checking for Common API Tokens and Access Tokens..."
COMMON_TOKEN_MATCHES=$(grep -r -E '(github_pat_[a-zA-Z0-9_]{82}|ghp_[a-zA-Z0-9]{36}|gho_[a-zA-Z0-9]{36}|ghu_[a-zA-Z0-9]{36}|ghs_[a-zA-Z0-9]{36}|ghr_[a-zA-Z0-9]{36}|ya29\.[a-zA-Z0-9_-]{68}|[a-zA-Z0-9]{32}-[a-zA-Z0-9]{16}-[a-zA-Z0-9]{16}-[a-zA-Z0-9]{16}-[a-zA-Z0-9]{32}|xoxb-[0-9]{11,12}-[0-9]{11,12}-[a-zA-Z0-9]{24}|xoxa-[0-9]{11,12}-[0-9]{11,12}-[a-zA-Z0-9]{24}|xoxp-[0-9]{11,12}-[0-9]{11,12}-[a-zA-Z0-9]{24}|xoxr-[0-9]{11,12}-[0-9]{11,12}-[a-zA-Z0-9]{24})' \
    --exclude-dir=.git \
    --exclude-dir=node_modules \
    --exclude-dir=vendor \
    --exclude-dir=.terraform \
...
if [ -n "$COMMON_TOKEN_MATCHES" ]; then
    # Check if matches are only in gitignored files
    GITIGNORED_COMMON_MATCHES=$(echo "$COMMON_TOKEN_MATCHES" | grep -E '\./configs/credentials\.json|\./\.env' || true)
    NON_GITIGNORED_COMMON_MATCHES=$(echo "$COMMON_TOKEN_MATCHES" | grep -v -E '\./configs/credentials\.json|\./\.env' || true)
    
    if [ -n "$NON_GITIGNORED_COMMON_MATCHES" ]; then
        report_issue "Common API Tokens found in tracked files:"
        echo "$NON_GITIGNORED_COMMON_MATCHES"
        echo ""
    fi
    
    if [ -n "$GITIGNORED_COMMON_MATCHES" ]; then
        report_warning "Common API Tokens found in gitignored files (this is expected):"
        echo "$GITIGNORED_COMMON_MATCHES"
        echo ""
    fi
    
    if [ -z "$NON_GITIGNORED_COMMON_MATCHES" ]; then
        report_success "Common API Tokens only found in gitignored files (safe)"
    fi
else
    report_success "No Common API Tokens found in tracked files"
fi

echo "9. Checking for sensitive files that should be gitignored..."
SENSITIVE_FILES=()
...
echo "10. Checking git status for staged sensitive files..."
...
echo "11. Checking for hardcoded secrets in environment variable assignments..."
