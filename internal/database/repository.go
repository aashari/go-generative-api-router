...
// GenerativeVendorLog operations

// CreateGenerativeVendorLog inserts a new generative vendor log entry
func (r *Repository) CreateGenerativeVendorLog(ctx context.Context, log *GenerativeVendorLog) error {
	if log.ID.IsZero() {
...
func (r *Repository) GetGenerativeVendorLogs(ctx context.Context, filter GenerativeVendorLogFilter) ([]GenerativeVendorLog, error) {
	...
	if filter.Model != "" {
		mongoFilter["model"] = filter.Model
	}
	if filter.RequestID != "" {
		mongoFilter["request_id"] = filter.RequestID
	}
...
// GenerativeVendorLogFilter represents filtering options for generative vendor logs
type GenerativeVendorLogFilter struct {
