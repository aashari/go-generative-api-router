...
// GenerativeVendorLog represents complete vendor request/response logging
// Stores full request and response data without obfuscation or truncation
type GenerativeVendorLog struct {
	ID          primitive.ObjectID `bson:"_id,omitempty" json:"id"`
	
	// Model and credential information
	Model      string `bson:"model" json:"model"`           // The model used (e.g., "gpt-4o", "gemini-2.0-flash")
	Credential string `bson:"credential" json:"credential"` // The credential/API key used (full value)
	
	// Complete request and response payloads
	RequestPayload  string `bson:"request_payload" json:"request_payload"`   // Full JSON request sent to vendor
	ResponsePayload string `bson:"response_payload" json:"response_payload"` // Full JSON response from vendor
	
	// Timestamp
	CreatedAt time.Time `bson:"created_at" json:"created_at"`
	
	// Additional metadata for context
	Vendor      string `bson:"vendor" json:"vendor"`           // Vendor name (openai, gemini, etc.)
	RequestID   string `bson:"request_id" json:"request_id"`   // Request ID for correlation
	StatusCode  int    `bson:"status_code" json:"status_code"` // HTTP status code
	DurationMs  int64  `bson:"duration_ms" json:"duration_ms"` // Request duration in milliseconds
	Environment string `bson:"environment" json:"environment"` // Environment (dev, prod, etc.)
}
