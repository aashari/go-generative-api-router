...
Practical guide for querying and monitoring production workloads for the Generative API Router service deployed as `xyz-aduh-genapi`.
...
```bash
export AWS_PROFILE=836322468413 AWS_REGION=ap-southeast-3 AWS_CLUSTER_PROD=prod-xyz-aduh-genapi AWS_SERVICE_PROD=prod-xyz-aduh-genapi && echo "✅ Environment loaded" | cat
```
...
**Quick Service Check:**
```bash
curl -f https://genapi.aduh.xyz/health && echo "✅ Service responding" || echo "❌ Service down" | cat
```
